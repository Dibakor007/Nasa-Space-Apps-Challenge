...
const SuggestedTopics: React.FC<SuggestedTopicsProps> = ({ topics, onTopicClick }) => {
    ...
    const row1Topics = useMemo(() => [...uniqueTopics].sort(() => 0.5 - Math.random()), [uniqueTopics]);
    const row2Topics = useMemo(() => [...uniqueTopics].sort(() => 0.5 - Math.random()), [uniqueTopics]);
    const row3Topics = useMemo(() => [...uniqueTopics].sort(() => 0.5 - Math.random()), [uniqueTopics]);

    if (!uniqueTopics.length) {
        return (
            <div className="bg-white/90 dark:bg-space-dark/70 p-6 rounded-lg border border-gray-200 dark:border-space-blue/30 text-center text-gray-500 dark:text-space-text-dim">

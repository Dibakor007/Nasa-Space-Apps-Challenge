...
import { getAiSummary, getExtendedAiSummary, getMetadata } from '../services/aiService';
...
import { AdvancedFilters, DetailedReportItem, AiSearchResult, SearchMetadata } from '../types';
  ...
  useEffect(() => {
    ...
    getMetadata()
      .then((data) => {
        if (!isActive) return;
        setMetadata(data);
        if (data.topicSuggestions?.length) {
          setTopicSuggestions(data.topicSuggestions);
          return;
        }
        const fallbackTopics = Array.from(new Set(data.categories.flatMap(cat => cat.items)));
        setTopicSuggestions(fallbackTopics);
      })
      .catch((err) => {
        if (!isActive) return;
        console.error('Failed to load metadata', err);
        setMetadataError(err instanceof Error ? err.message : 'Unable to load topic suggestions.');
      });
    return () => {
      isActive = false;
    };
          ...
          <div className="space-y-4">
            {!searchResult && !isLoading && !error && !noResults && (
               <div className="relative z-20">
                  <SuggestedTopics topics={topicSuggestions} onTopicClick={handleTopicSearch} />
                  {metadataError && (
                    <ErrorMessage
                      type="info"
            ...
            <AdvancedFilterControls
              ...
              organismOptions={metadata?.organismOptions ?? []}
              missionOptions={metadata?.missionOptions ?? []}
              researchAreaOptions={metadata?.researchAreaOptions ?? []}
              publicationTypeOptions={metadata?.publicationTypeOptions ?? []}
            />
          </div>

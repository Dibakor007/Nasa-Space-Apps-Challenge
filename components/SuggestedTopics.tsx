...
const TopicButton: React.FC<{ topic: string, onTopicClick: (topic: string) => void }> = ({ topic, onTopicClick }) => (
    <button
        ...
        className="flex-shrink-0 mx-2 px-4 py-2 text-sm rounded-full transition-colors bg-blue-100 text-blue-800 dark:bg-space-blue/50 dark:text-space-text-dim hover:bg-blue-200 dark:hover:bg-space-blue hover:scale-105 transform"
...
const SuggestedTopics: React.FC<SuggestedTopicsProps> = ({ onTopicClick }) => {
    ...
    return (
        <div className="bg-white dark:bg-space-dark/50 p-6 rounded-lg shadow-md dark:shadow-lg border border-gray-200 dark:border-space-blue/30">
            <div className="relative group overflow-hidden space-y-4 py-4 [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
